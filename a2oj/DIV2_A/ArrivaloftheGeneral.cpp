...
void run_cases(){
    int n; cin >> n;
    vi arr; revi(n, arr);
    int min_index = 0, cnt = 0, max_index = 0;
    F(n){
    ...
    while(true){
        if(arr[n - 1] == *min_element(arr.begin(), arr.end())){
            break;
        }
        swap(arr[min_index], arr[min_index + 1]);
        min_index++;
        cnt++;
    }
    ...
    while(true){
        if(arr[0] == *max_element(arr.begin(), arr.end())){
            break;
        }
